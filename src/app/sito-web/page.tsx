              ...
              {types.map((rt) => (
                <button key={rt.id} onClick={() => go()} className="group overflow-hidden rounded-xl border border-line bg-surface text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                  ...
                  <div className="p-3">
                    ...
                    {(rt.amenities ?? []).length > 0 && <div className="mt-2 flex flex-wrap items-center gap-2 text-dim">{(rt.amenities ?? []).slice(0, 8).map((a) => <span key={a} title={a} aria-label={a}>{amenityIcon(a)}</span>)}</div>}
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {offers.map((p) => (
                <div key={p.id} className="flex flex-col rounded-xl border p-4" style={{ borderColor: `color-mix(in srgb, ${accent} 40%, var(--line))`, backgroundColor: `color-mix(in srgb, ${accent} 6%, transparent)` }}>
                  ...
                  <button onClick={() => go(p.code ? `&promo=${encodeURIComponent(p.code)}` : "")} className="group mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:opacity-90 active:scale-[0.98]" style={{ backgroundColor: accent }}>{T("Prenota")} <span className="transition-transform group-hover:translate-x-0.5">→</span></button>
                </div>
              ))}
            </div>
          </section>
