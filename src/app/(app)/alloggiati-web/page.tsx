...
export default function AlloggiatiWebPage() {
  ...
  const [confirmSend, setConfirmSend] = useState(false); // conferma invio definitivo alla Questura
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-line bg-surface p-3 shadow-sm">
        ...
        <button onClick={() => setConfirmSend(true)} disabled={!!busy || readyCount === 0} className="rounded-lg bg-focus px-3 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50" title="Invia subito alla Questura, senza aspettare l'orario automatico">{busy === "send" ? "Invio…" : `Invia le pronte (${readyCount})`}</button>
      ...
      {confirmSend && (
        <ConfirmDialog
          title="Invio alla Questura"
          message={<>Stai per inviare <b className="text-txt">{readyCount}</b> {readyCount === 1 ? "schedina pronta" : "schedine pronte"} al Portale Alloggiati.</>}
          warning={<>L&apos;invio è <b>definitivo</b>: una volta inviate non è possibile annullarle.{errCount > 0 ? ` Le ${errCount} schedine con errori/incomplete NON verranno inviate: correggile prima.` : ""}</>}
          confirmLabel={`Invia ${readyCount} alla Questura`}
          busy={busy === "send"}
          onConfirm={async () => { await call("send", "send"); setConfirmSend(false); }}
