...
export default function PrenotazioniPage() {
  ...
  const renderCells = (b: typeof sorted[number], indent = false) => {
    ...
    return (<>
      ...
      <td className="px-3 py-2.5"><span className="inline-flex items-center gap-1.5"><ChannelLogo channel={b.channel} size={16} title={b.channel === "direct" ? "xenora.it" : ch.label} /><span className="text-xs text-dim">{b.channel === "direct" ? "xenora.it" : ch.label}</span></span></td>
              ...
              <div key={gid} className="rounded-xl border border-line bg-surface shadow-sm">
                <button onClick={() => toggleGroup(gid)} className="block w-full p-3 text-left active:bg-wash">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <ChannelLogo channel={b.channel} size={18} title={b.channel === "direct" ? "xenora.it" : ch.label} />
                  </div>
