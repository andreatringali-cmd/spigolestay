...
const ONLINE = [
  { name: "Andrea Tringali", role: "Titolare", you: true },
  { name: "Greta", role: "Reception" },
];
      ...
      <button onClick={() => setOpen((o) => !o)} title={t("Utenti online")} className="flex items-center gap-2 rounded-lg border border-line px-2 py-1 transition hover:bg-wash">
        <div className="flex -space-x-2">
          {ONLINE.slice(0, 3).map((u, i) => (
            <span key={i} className="grid h-6 w-6 place-items-center rounded-full border-2 border-surface text-[10px] font-bold text-white" style={{ backgroundColor: AVATAR[i % AVATAR.length] }}>{initials(u.name)}</span>
          ))}
        </div>
          ...
          {ONLINE.map((u, i) => (
            <div key={i} className="flex items-center gap-2 rounded-lg px-2 py-1.5">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-dim">{t(u.role)}</div>
              </div>
            </div>
          ))}
        </div>
