...
// Adempimenti di oggi (compliance) — vive nella Dashboard. Mostra solo le categorie che
// richiedono attenzione; se è tutto a posto, una riga verde. Sostituisce la vecchia pagina /adempimenti.
...
export default function AdempimentiToday() {
  ...
  const Tile = ({ n, label, tone, action, onClick, children }: { n: number; label: string; tone: string; action: string; onClick: () => void; children?: React.ReactNode }) => (
    <Card className="flex flex-col">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-mono text-3xl font-bold" style={{ color: tone }}>{n}</div>
  ...
  const tiles = [
    arrivalsNoCheckin.length > 0 && (
      <Tile key="chk" n={arrivalsNoCheckin.length} label="Arrivi senza check-in online" tone="var(--warn)" action="Prenotazioni" onClick={() => router.push("/prenotazioni")}>
    ...
    schedRisk.length > 0 && (
      <Tile key="sch" n={schedRisk.length} label="Schedine da inviare (rischio 24h)" tone="var(--err)" action="Alloggiati Web" onClick={() => router.push("/alloggiati-web")}>
        <div>di cui in arrivo oggi: {schedToday.length}</div>
      </Tile>
    ),
    istatPending > 0 && <Tile key="ist" n={istatPending} label="Movimenti ISTAT da inviare" tone="var(--warn)" action="ISTAT · Turist@t" onClick={() => router.push("/istat")} />,
    docsRejected.length > 0 && (
      <Tile key="rej" n={docsRejected.length} label="Documenti scartati dallo SdI" tone="var(--err)" action="Documenti" onClick={() => router.push("/documenti")}>
        {docsRejected.slice(0, 4).map((d) => <div key={d.id} className="truncate">{d.number_label} · {d.counterpart?.name ?? ""}</div>)}
      </Tile>
    ),
    docsUnpaid.length > 0 && (
      <Tile key="unp" n={docsUnpaid.length} label="Documenti da incassare" tone="var(--focus)" action="Scadenzario" onClick={() => router.push("/scadenzario-incassi")}>
    ...
    passiveOverdue.length > 0 && (
      <Tile key="pas" n={passiveOverdue.length} label="Fatture fornitori scadute" tone="var(--err)" action="Fatture passive" onClick={() => router.push("/fatture-passive")}>
  ...
  return (
    <div className="mb-6">
      <SectionTitle>Adempimenti oggi</SectionTitle>
      {tiles.length === 0 ? (
        <Card className="mt-2"><p className="text-sm font-medium text-[color:var(--ok)]">✓ Tutto in ordine per oggi.</p></Card>
      ) : (
        <div className="mt-2 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{tiles}</div>
      )}
    </div>
  );
}
