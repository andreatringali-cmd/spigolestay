...
(function () {
  ...
  (function () {
    ...
    var hasGuest = ["c", "camera", "k", "pk", "park", "p", "d", "tax"].some(function (x) { return params.has(x); });
    if (hasGuest) {
      var guest = {
  ...
  function renderHome(t) {
    ...
    if (t.home.welcome && t.home.welcome.length) {
      ...
      if (t.home.welcomeTitle) html += '<h2 class="welcome-heading">' + esc(fill(t.home.welcomeTitle)) + "</h2>";
  ...
  function renderSection(t, s) {
    ...
    if (s.photos && s.photos.length) {
      ...
      if (heroImg) html += '<img class="section-hero" src="' + esc(heroImg) + '" alt="' + esc(fill(s.title)) + '" loading="lazy">';
    }
    ...
    // Galleria foto subito dopo i passaggi (flag gallery)
